export type ColourPoint={hue:number;saturation:number;value?:number};
...
export function sampleColours(pixels:Uint8ClampedArray):ColourPoint[]{
 const bins=Array.from({length:36},()=>({weight:0,x:0,y:0,s:0,v:0,count:0}));let count=0;
 for(let i=0;i<pixels.length;i+=4){if(pixels[i+3]<128)continue;const p=rgbToHsv(pixels[i],pixels[i+1],pixels[i+2]);if(p.saturation<.10||p.value<.08)continue;
  const b=bins[Math.floor(p.hue/10)%36],w=p.saturation*p.value;b.weight+=w;b.x+=Math.cos(p.hue*Math.PI/180)*w;b.y+=Math.sin(p.hue*Math.PI/180)*w;b.s+=p.saturation*w;b.v+=p.value*w;b.count++;count++;
 }
 const groups=bins.map((_,i)=>{const neighbours=[bins[(i+35)%36],bins[i],bins[(i+1)%36]];return neighbours.reduce((a,b)=>({weight:a.weight+b.weight,x:a.x+b.x,y:a.y+b.y,s:a.s+b.s,v:a.v+b.v,count:a.count+b.count}),{weight:0,x:0,y:0,s:0,v:0,count:0})}).sort((a,b)=>b.weight-a.weight);
 ...
 for(const g of groups){if(g.weight===0||g.weight<groups[0].weight*.06||g.count<count*.008)continue;
  ...
  if(selected.every(p=>hueDistance(p.hue,hue)>32))selected.push({hue,saturation:g.s/g.weight,value:g.v/g.weight});if(selected.length===3)break;
 }
...
export function swatchPixels(hexes:string[]){return new Uint8ClampedArray(hexes.flatMap(hex=>[parseInt(hex.slice(1,3),16),parseInt(hex.slice(3,5),16),parseInt(hex.slice(5,7),16),255]))}
export function pixelsToHex(pixels:Uint8ClampedArray){const hexes:string[]=[];for(let i=0;i<pixels.length;i+=4)hexes.push('#'+[pixels[i],pixels[i+1],pixels[i+2]].map(v=>v.toString(16).padStart(2,'0')).join(''));return hexes}
