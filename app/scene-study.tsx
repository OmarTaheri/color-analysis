...
export default function SceneStudy({scene}:{scene:typeof scenes[number]}){
 ...
 return <div className="scene-study">
  ...
  <div className="adjustment-heading"><h3>Try changing the colours</h3><button type="button" className="colour-reset" data-visible={changed} disabled={!changed} aria-hidden={!changed} tabIndex={changed?0:-1} onClick={()=>{setTargets(colours.map(p=>({...p})));setDragging(null);setResetKey(k=>k+1);wheel.current?.querySelector<HTMLButtonElement>("button")?.focus({preventScroll:true})}}><span key={resetKey} aria-hidden="true">↺</span> Reset</button></div>
  ...
  <div className="adjustment-controls">
   <div className="hue-control"><div ref={wheel} className="adjustment-wheel" data-dragging={dragging!==null} data-edited={changed} role="group" aria-label="Picture colours">
    <div className="adjustment-wheel-hole" aria-hidden="true"/>
    <svg viewBox="0 0 100 100" aria-hidden="true">{targets.map((p,i)=>{const pos=pointPosition(p);return <line key={i} x1="50" y1="50" x2={pos.x} y2={pos.y}/>})}</svg>
