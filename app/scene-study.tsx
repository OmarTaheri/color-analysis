...
import {pointFromPosition,pointPosition,sampleColours,recolourPixels,rgbToHsv,hueDistance,type ColourPoint} from './color-adjustments';
...
export default function SceneStudy({scene,palette}:{scene:typeof scenes[number];palette:ReactNode}){
 ...
 const [targets,setTargets]=useState<ColourPoint[]>([]);
 ...
 const prepare=()=>{
  ...
  try{const scale=Math.min(1,900/im.naturalWidth);c.width=Math.round(im.naturalWidth*scale);c.height=Math.round(im.naturalHeight*scale);
   const ctx=c.getContext('2d');if(!ctx)throw new Error('Canvas unavailable');ctx.drawImage(im,0,0,c.width,c.height);source.current=ctx.getImageData(0,0,c.width,c.height);const sampled=sampleColours(source.current.data);if(scene.image===147){const pink=rgbToHsv(123,71,86);if(sampled.every(p=>hueDistance(p.hue,pink.hue)>20))sampled.push({hue:pink.hue,saturation:pink.saturation})}setColours(sampled);setTargets(sampled);setReady(true);
 ...
 return <div className="scene-study">
  <div className="scene-preview"><img ref={image} className="dialog-image" src={'/stills/'+scene.image+'.webp'} alt={scene.location} onLoad={prepare} style={{display:changed&&ready?'none':'block'}}/><canvas ref={canvas} role="img" aria-label={scene.location+' with your colour changes'} style={{display:changed&&ready?'block':'none'}}/><div className="popup-palette"><h4>Main colours in the original picture</h4>{palette}</div></div>
